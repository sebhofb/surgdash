    ...
    const RELEASES = [
        {
            ...
            date: 'September 2026',
            items: [
                { t: 'UNITAR course reports', d: 'One Excel file per course, for uploading a course to UNITAR as an event, carrying the GSF logo. Two sheets: the totals with the breakdown by country, career stage, gender, organisation type and profession \u2014 a count and a percentage, nothing more \u2014 and the anonymised participant list. Pick a reporting period (a year, or any range of months) and the file holds only the learners who enrolled in it. The summary gives the month the course went live, read from the enrolment curve rather than the creation date. How it all works lives on the Methodology page rather than in the file.' },
                { t: 'Private courses', d: 'Mark a course private on its page \u2014 in-country workshops and other closed cohorts. Private courses are left out when you generate UNITAR reports for every course, unless you tick to include them. The classification travels with the data, so the whole team sees the same thing.' },
