    ...
    const RELEASES = [
        {
            ...
            date: 'September 2026',
            items: [
                { t: 'Passwords travel with the app, not the Sheet', d: 'The edit and reporting passwords live on each computer and never go through Google Sheets, so a password changed in Settings only ever changed that machine. From this release the built-in passwords are the ones in use, and a future change reaches every install with the update. The built-in reporting password is now the current one; a computer still holding the old one drops it the first time it is typed and says so.' },
                { t: 'Forum activity on every course page', d: 'A chart of what learners write in each course\u2019s discussion space: posts a month, with a line for how many different people wrote them. Ten posts from one person is a different course from ten people posting once. Read from LearnWorlds by Data Sync \u2192 Forum activity; counts only, no post text and no learner identity is stored. Posts only \u2014 the platform does not report replies.' },
                { t: 'UNITAR course reports use UNITAR\u2019s own upload template', d: 'The report is now their EMS workbook: their Participants sheet filled in, their Default Values sheet untouched, and a SURGhub summary added in front under the GSF logo. Gender, nationality and organisation type are written as UNITAR\u2019s own codes, read from the template itself; where a learner stated nothing, their \u201cUnreported\u201d code is used. Surname, first name and email are left blank \u2014 UNITAR marks them required, but filling them would identify every learner \u2014 and only the completion certificate is reported.' },
                { t: 'UNITAR quality-assessment forms', d: 'Their QA form, filled per course for a year you choose, as a Word document in their own layout. Twenty rows: title, dates, partner, public link, fee, background, the course summary, this year\u2019s participation and countries. Language and focal point are left for you, and learning objectives are filled only where the course itself states them. One per course, or one for every course at once.' },
                { t: 'Course summaries', d: 'Data Sync can now fetch each course\u2019s summary text from LearnWorlds. SURGdash held only numbers before, which is what the QA forms were missing.' },
                { t: 'Private courses', d: 'LearnWorlds already marks courses free, private or draft, so in-country workshops and unreleased courses classify themselves. A Private tick on the course page and in the Directory overrides that where the platform\u2019s answer is not the reporting answer. Private courses are left out when you generate reports for every course, unless you tick to include them.' },
