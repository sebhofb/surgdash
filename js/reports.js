...
(function() {
    ...
    Object.assign(window.App, {
        ...
        async _assembleReportData(providerName, opts) {
            ...
            // report machinery then runs unchanged on a one-course pSnap.
            const courseOnly = (!platform && opts.course) ? String(opts.course) : null;
            ...
            const pSnap = platform ? snapData.slice() : snapData.filter(d => d.Provider === providerName && (!courseOnly || d.Course === courseOnly));
            ...
            let provTimeline = {};
            const historyData = platform ? this.getAnalyticsHistory() : this.getAnalyticsHistory().filter(d => d.Provider === providerName && (!courseOnly || d.Course === courseOnly));
            ...
            try {
                ...
                if (aw) {
                    // Course mode: provider-level awards belong to the provider's whole
                    // catalogue, not this course — omit them; course awards still show.
                    if (!courseOnly) {
            ...
            // Course mode: the header link should point at the course, not the provider page.
            if (courseOnly) {
                try {
            ...
            return {
                ...
                courseName: courseOnly, isCourse: !!courseOnly,
        ...
        async _buildReportHtml(providerName, opts) {
            opts = opts || {};
            const D = await this._assembleReportData(providerName, opts);
            if (!D) return null;
            const {
...
<div class="page">
  <div class="header">
    <div><h1>${esc(D.isCourse ? D.courseName : providerName)}</h1><p style="color:#4389C8;font-weight:600;margin:4px 0 0;font-size:14px">${D.isCourse ? 'A course by ' + esc(providerName) + ' &middot; SURGhub Course Report' : 'SURGhub Performance Report'}</p></div>
  ...
  <div class="summary-box">
    <strong>Summary:</strong> ${D.isCourse
        ? '<strong>' + esc(D.courseName) + '</strong> (a course by ' + esc(providerName) + ') is live on SURGhub,'
        : esc(providerName) + ' has <strong>' + pSnap.length + ' active course' + (pSnap.length !== 1 ? 's' : '') + '</strong> on SURGhub,'}
...
<\/script>
        ...
        async _buildDarkReportHtml(providerName, platform, opts) {
            platform = !!platform;
            opts = opts || {};
            const D = await this._assembleReportData(providerName, { platform, course: opts.course });
            if (!D) return null;
            // Course mode: the report's subject is the course; the provider becomes context.
            const subjectName = D.isCourse ? D.courseName : providerName;
            const esc = (s) => this.escapeHtml(s);
            ...
            const milestone = provBits.length ? ('Milestones: ' + subjectName + ' ' + provBits.join(' and ')) : '';
...
<title>${esc(subjectName)} — SURGhub ${platform ? 'Platform' : (D.isCourse ? 'Course' : 'Provider')} Report</title>
...
<div id="page-main">
  <div style="border:1px solid var(--border);border-radius:5px;padding:22px 24px;margin-bottom:24px;background:linear-gradient(135deg,#4389C80D,#002033 62%)">
    ...
    <div style="min-width:0;flex:1">
      ...
      <h1 style="margin:0;font-size:30px;font-weight:700;color:#eef4f9;line-height:1.12;letter-spacing:-0.01em">${D.providerUrl ? '<a class="shl" href="' + esc(D.providerUrl) + '" target="_blank" rel="noopener">' + esc(subjectName) + ' <span style="font-size:15px;color:#6d8ba3">&#8599;</span></a>' : esc(subjectName)}</h1>
      <p style="margin:12px 0 0;font-family:var(--mono);font-size:10.5px;font-weight:500;letter-spacing:.08em;text-transform:uppercase;color:#6d8ba3;display:inline-flex;align-items:center;gap:7px"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#4389C8"></span>${platform ? 'Platform Performance Report' : (D.isCourse ? 'Course Performance Report &middot; A course by ' + esc(providerName) : 'Provider Performance Report')} &middot; ${esc(D.reportDate)}<span id="hdr-period"></span></p>
      <p style="margin:14px 0 0;font-size:13px;color:#9fb3c8;line-height:1.6;font-weight:300">${platform
        ...
        : (D.isCourse
            ? '<strong style="color:#eef4f9">' + esc(D.courseName) + '</strong> &mdash; a course by <strong style="color:#eef4f9">' + esc(providerName) + '</strong> &mdash; is live on SURGhub, reaching <strong style="color:#eef4f9">' + fmt(D.totalLrn) + ' learners</strong>' + (D.certRate != null ? ' with a <strong style="color:#eef4f9">' + D.certRate + '% certification rate</strong>' : '') + '.' + (D.totalResp > 0 ? ' Across ' + fmt(D.totalResp) + ' survey responses, the average rating is <strong style="color:#eef4f9">' + D.avgRating + ' / 5</strong>.' : '')
  ...
  ${(function(){
      ...
      if (provBits.length) {
          mile = '<p style="margin:0 0 6px;font-family:var(--mono);font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:var(--accent)">Milestones</p>'
            + '<p style="margin:0 0 24px;font-size:12.5px;color:#d4dde7;line-height:1.65">&#127942; ' + esc(subjectName) + ' ' + provBits.join(' and ') + '.</p>';
      }
      let geo = '';
      if (pc.length) {
...
var PROV=${JSON.stringify(subjectName)};
...
<\/script>
        ...
        _buildUsersWorkbook(providerName, anonUsers, courseOnly) {
            const provCourses = this.getAnalyticsSnap().filter(d => d.Provider === providerName && (!courseOnly || d.Course === courseOnly)).map(d => d.Course);
        ...
        async _buildFeedbackWorkbook(providerName, courseOnly) {
        ...
        // Course page button — exports the currently selected course. Deliberately takes
        // no inline-HTML string args: course/provider names with quotes or apostrophes
        // would break an onclick string literal (escapeHtml entity-encodes them and the
        // browser decodes entities before the JS runs).
        async exportCurrentCoursePackage() {
            const course = this.selectedCourse;
            if (!course) return alert('No course selected.');
            const row = (this.getAnalyticsSnap() || []).find(d => d.Course === course);
            return this.exportCoursePackage(row ? row.Provider : '', course);
        },

        // Course page: one course's package → {folder}/{provider}/{course}/.
        async exportCoursePackage(providerName, courseName) {
            if (!providerName || !courseName) return alert('No course selected.');
            const folder = await electronAPI.invoke('pick-folder');
            if (!folder) return;
            const dateStr = new Date().toISOString().split('T')[0];
            const anonUsers = await this._getAnonUsers();
            this._showReportProgress('Building course package for ' + courseName + '…');
            try {
                const s = await this._writeCoursePackage(providerName, courseName, folder, dateStr, anonUsers);
                this._hideReportProgress();
                alert('Course package for ' + courseName + ':\n\n' +
                    (s.pdf ? '✓' : '✗') + ' PDF report\n' +
                    (s.html ? '✓' : '✗') + ' Web report (.html)\n' +
        ...
        async exportProviderCoursePackages(providerName) {
            ...
            if (courses.length === 0) return alert('No course data for ' + providerName + '.');
            const folder = await electronAPI.invoke('pick-folder');
            if (!folder) return;
            const dateStr = new Date().toISOString().split('T')[0];
            const anonUsers = await this._getAnonUsers();
            ...
            for (let i = 0; i < courses.length; i++) {
                if (this._reportCancelled) break;
                this._showReportProgress('Course package ' + (i + 1) + '/' + courses.length + ': ' + courses[i], true);
                try {
                    const s = await this._writeCoursePackage(providerName, courses[i], folder, dateStr, anonUsers);
                    if (s.pdf && s.html && s.users && s.feedback) ok++;
                    else if (s.pdf || s.html || s.users || s.feedback) partial++;
                    else failed++;
