        ...
        async generateImprovementSummary() {
            ...
            try {
                ...
                if (tiers.length >= 2) {
                    ...
                    const parsed = await this._claudeJSON(apiKey,
                        VOICE + '\n\nYou are given two sets of comments: LIC/LMIC learners (low- and lower-middle-income countries) and UMIC/HIC learners (upper-middle- and high-income countries). Find the REAL differences in what each set asks for — not the similarities, and not differences you cannot see in the text. contrast: 2-3 sentences naming what LIC/LMIC learners ask for that UMIC/HIC learners do not (or ask for far less). lowerIncome / higherIncome: 1-2 sentences each on that set\'s distinctive asks. actions: up to 4 changes that would specifically help the LIC/LMIC audience, each max 16 words. If the two sets look substantially the same, say that plainly instead of manufacturing a difference — that is a useful finding.\n\nIMPORTANT: your text is shown to readers who never see this prompt. Always write "LIC/LMIC learners" and "UMIC/HIC learners" (or "lower-income" / "higher-income" learners). NEVER write "Group A", "Group B", "the first set" or "the second set" — those labels are meaningless on screen.',
                        'LIC/LMIC LEARNERS — low and lower-middle income countries (' + lower.length + ' comments; topic mix: ' + share(lower) + ')\n' + this._corpusLines(lo.picked)
                        + '\n\n=====\n\nUMIC/HIC LEARNERS — upper-middle and high income countries (' + higher.length + ' comments; topic mix: ' + share(higher) + ')\n' + this._corpusLines(hi.picked),
