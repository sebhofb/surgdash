            ...
            body.innerHTML = `
                <div class="p-6 md:p-10 max-w-6xl mx-auto w-full fade-in">
                    ...
                    <div class="bg-gradient-to-br from-indigo-50 to-white border border-indigo-200 rounded-2xl p-6 mb-4 shadow-sm">
                        ...
                        <p class="text-xs text-slate-400 mt-2 italic">Run card 1 first — records are matched to the synced course list (the API by course id, the xlsx by name). <strong class="text-slate-500">Sync from API</strong> ingests any earlier run's receipt offline first, then continues at the paced rate with a checkpoint every 200 accounts; after the first full pass, runs are incremental (accounts active since the last run) and take minutes. A network blip or API error is retried in place for up to ~6 hours, so an overnight run keeps going; if a run does stop, the reason is shown next to the heading. The upload stays as a fallback.</p>
                    </div>
