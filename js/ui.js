                ...
                <div class="p-6 md:p-10 fade-in w-full max-w-4xl mx-auto">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border p-8 mb-8">
                        ...
                        <div class="border border-slate-200 rounded-xl p-4 mb-4 bg-slate-50/40 max-w-4xl">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                ...
                                <button onclick="App.exportAllCoursePackages()" class="flex items-center justify-center gap-2 py-3 px-4 bg-white border border-slate-200 text-gsf-prussian font-bold rounded-lg hover:border-gsf-boston hover:text-gsf-boston hover:shadow-md transition-all shadow-sm" title="The full tree: each provider folder gets its consolidated provider report files PLUS a subfolder per course (each with its own PDF + web report + anonymized users + anonymized feedback). Slow — one PDF per provider and per course.">
                                ...
                                <button onclick="App.exportMasterWorkbook()" class="flex items-center justify-center gap-2 py-3 px-4 bg-white border border-slate-200 text-gsf-prussian font-bold rounded-lg hover:border-gsf-boston hover:text-gsf-boston hover:shadow-md transition-all shadow-sm" title="One Excel with everything: Learners (one row per user — incl. NAME and EMAIL, so handle with care and delete those columns before wider sharing), Courses (full per-course profile), Providers, Countries, and a Monthly timeline — plus an About tab with definitions.">
                                    <i data-lucide="database" width="18" class="text-gsf-boston"></i> Master Data Export
                                </button>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border p-8 mb-8">
                        ...
                        <div class="flex flex-wrap items-end gap-3">
                            ...
                            <button onclick="ImpactShowcase.openPicker({ source: (document.getElementById('showcase-tsource')||{}).value, minScore: parseInt((document.getElementById('showcase-minscore')||{}).value, 10) })" class="flex items-center gap-2 py-2.5 px-4 bg-gsf-prussian text-white font-bold rounded-lg text-sm hover:bg-slate-900 transition-all shadow-sm">
                                <i data-lucide="sparkles" width="16"></i> Choose testimonials & build…
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto max-h-[500px] overflow-y-auto custom-scrollbar">
                            <table class="w-full text-left border-collapse text-sm table-fixed">
                                <thead class="sticky top-0 bg-white shadow-sm z-10"><tr class="border-b text-slate-500">
                                    ...
                                    <th class="py-3 px-3 font-medium w-[124px]" title="Courses started per month over the last 12 complete months. Colour and % compare the last 3 months with the 3 before.">Trend <span class="text-[10px] font-normal text-slate-400">12 mo</span></th>
                                ...
                                <tbody>
                                    ${(() => {
                                        ...
                                        return enriched.map(s => {
                                            ...
                                            <td class="py-2 px-3">${this.courseTrendSpark(s.Course, { width: 70, height: 20 })}</td>
                ...
                <div class="p-6 md:p-10 max-w-6xl mx-auto w-full fade-in">
                    ...
                    <div class="bg-gradient-to-br from-indigo-50 to-white border border-indigo-200 rounded-2xl p-6 mb-4 shadow-sm">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <button onclick="App.syncEnrolmentsFromApi()" class="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg shadow-sm transition-colors text-sm flex items-center gap-2" title="Fetch every learner's enrolments and progress straight from LearnWorlds — same data as the xlsx, no download needed. The first full pass is ~33 hours of API time run in sessions (it saves every 200 accounts and resumes); after that a few minutes."><i data-lucide="cloud-download" width="16"></i> Sync from API</button>
                            <button onclick="App.syncEnrolmentsFromApi({ mode: 'full' })" class="text-xs text-slate-500 hover:text-gsf-boston underline" title="Ignore the last run and re-fetch every account (~33 hours of API time, in sessions)">full re-sync</button>
                        ...
                        <p class="text-xs text-slate-400 mt-2 italic">Run card 1 first — records are matched to the synced course list (the API by course id, the xlsx by name). <strong class="text-slate-500">Sync from API</strong> ingests any earlier run's receipt offline first, then continues at the paced rate with a checkpoint every 200 accounts; after the first full pass, runs are incremental (only accounts that logged in since they were last fetched, and certificates only for courses with new completions) and take minutes. A network blip or API error is retried in place for up to ~6 hours, so an overnight run keeps going; if a run does stop, the reason is shown next to the heading. The upload stays as a fallback.</p>
                    </div>
